export declare const MyComponent: import('tamagui').TamaguiComponent<
  Omit<import('@tamagui/types-react-native').ViewProps, 'display' | 'children'> &
    import('tamagui').RNWViewProps &
    import('tamagui').TamaguiComponentPropsBase &
    import('@tamagui/core').WithThemeValues<import('@tamagui/core').StackStylePropsBase> &
    import('@tamagui/core').WithShorthands<
      import('@tamagui/core').WithThemeValues<import('@tamagui/core').StackStylePropsBase>
    > &
    Omit<{}, 'blue'> & {
      blue?: boolean | undefined
    } & import('@tamagui/core').MediaProps<
      Partial<
        Omit<import('@tamagui/types-react-native').ViewProps, 'display' | 'children'> &
          import('tamagui').RNWViewProps &
          import('tamagui').TamaguiComponentPropsBase &
          import('@tamagui/core').WithThemeValues<import('@tamagui/core').StackStylePropsBase> &
          import('@tamagui/core').WithShorthands<
            import('@tamagui/core').WithThemeValues<import('@tamagui/core').StackStylePropsBase>
          > &
          Omit<{}, 'blue'> & {
            blue?: boolean | undefined
          }
      >
    > &
    import('@tamagui/core').PseudoProps<
      Partial<
        Omit<import('@tamagui/types-react-native').ViewProps, 'display' | 'children'> &
          import('tamagui').RNWViewProps &
          import('tamagui').TamaguiComponentPropsBase &
          import('@tamagui/core').WithThemeValues<import('@tamagui/core').StackStylePropsBase> &
          import('@tamagui/core').WithShorthands<
            import('@tamagui/core').WithThemeValues<import('@tamagui/core').StackStylePropsBase>
          > &
          Omit<{}, 'blue'> & {
            blue?: boolean | undefined
          }
      >
    >,
  import('tamagui').TamaguiElement,
  import('tamagui').StackPropsBase,
  {
    blue?: boolean | undefined
  }
>
//# sourceMappingURL=MyComponent.d.ts.map
